...
NormalData ConvertSurfaceDataToNormalData(SurfaceData surfaceData)
{
    ...
    // Note: When we are in the prepass (depth only or velocity) and we need to export the normal/roughness,
    // If the fragment that we are processing has a clear cloat mask to 1, we want to export the clear coat's roughness and the geometric normal
    // instead of the base layer's roughness and the shader normal. 
